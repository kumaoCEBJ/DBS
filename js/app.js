    ...
    if (myWorker === null) {
        ...
        myWorker.onmessage = function (e) {
            ...
            switch (message.type) {
                ...
                case 'result':
                    if (dotNetHelper && message.payload) {
                        const resultJson = JSON.stringify(message.payload);
                        dotNetHelper.invokeMethodAsync('HandleWorkerResult', resultJson);
                    }
                    break;
                case 'torehan':
                    if (dotNetHelper && message.payload) {
                        const resultJson = JSON.stringify(message.payload);
                        dotNetHelper.invokeMethodAsync('HandleWorkerResultTorehan', resultJson);
                    }
                    break;
                case 'error':
                    if (dotNetHelper && message.payload) {
